...
from flask import Blueprint, current_app as app, render_template, request, url_for
import requests

bp = Blueprint(
    "register_with_discovery", __name__, url_prefix="/register_with_discovery"
)
...
def register_with_discovery():
    if request.method == "GET":
        ...
        urls_available_list = [
            url_for("localize.image_localize", name=map_name)[
                1:
            ]  # Remove the leading slash
            for map_name in map_names_list
        ]
        return render_template(
            "register_with_discovery.html", urls_available_list=urls_available_list
        )
